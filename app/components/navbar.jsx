...
export default function NavBar() {
  ...
  return (
    <nav className="h-screen w-full flex flex-col justify-center items-center min-[805px]:flex-row min-[805px]:justify-between min-[805px]:bg-color60 min-[805px]:z-50 min-[805px]:h-[14%] min-[805px]:fixed">
      <Image
        className="w-3/4 h-auto order-2 min-[805px]:w-32 min-[805px]:order-first"
        src={logo}
        alt="logo"
        draggable="false"
        loading="eager"
      ...
      <button
        className={`fixed top-0 left-0 ${
          isToggled ? "hidden" : ""
        } cursor-default h-full w-1/2 bg-color60 bg-opacity-30 transform transition-transform duration-500 min-[805px]:hidden`}
        onClick={handleClick}
      ></button>
      <button
        onClick={handleClick}
      ...
      <ul
        className={`${headerFont.className} 
          ...
          `}
      >
        <li className="hover:text-color10 duration-300 min-[805px]:w-1/3 min-[805px]:text-center">
